...
const editButtons   = document.getElementById("editButtons"); // nuevo contenedor para export/import
...
function toggleEditMode(){
  ...
  addItemBtn.style.display = editMode ? "block" : "none"; // mantener el botón añadir
...
function render(){
  ...
  // Ya no necesitamos controlar addItemBtn aquí porque toggleEditMode lo hace
  editBtn.textContent = editMode ? "↩️ Volver" : "✏️ Editar";
...
function buildWhatsAppText(){
  ...
  categories.forEach(cat => {
    const lines = cart.filter(c => {
      const it = items.find(i => i.name === c.name && i.cat === cat);
      return it;
    });
    if(lines.length){
      txt += cat.toUpperCase() + "\n";
...
function importData(event){
  ...
  reader.onload = e => {
    try {
      ...
      alert("Error leyendo el archivo ⚠️");
    }
  };
